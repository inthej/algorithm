...
function isBabbling(str) {
  if (
    ValueUtils.nonEmpty(
      str
        .split(AppTypes.BabblingType.aya)
        .flatMap((el) => el.split(AppTypes.BabblingType.ye))
        .flatMap((el) => el.split(AppTypes.BabblingType.woo))
        .flatMap((el) => el.split(AppTypes.BabblingType.ma))
        .join(''),
    )
  ) {
    return false
...
const AppTypes = {}
AppTypes.BabblingType = Object.freeze({
  aya: 'aya',
  ye: 'ye',
  woo: 'woo',
  ma: 'ma',
