...
function solution(players, callings) {
  ...
  callings.forEach(player => {
    const playerIndex = playerIndexMap.get(player);
    const targetIndex = playerIndex - 1;
    const target = players[targetIndex]

    ArrayUtils.swap(players, playerIndex, targetIndex);

    playerIndexMap.set(player, targetIndex)
    playerIndexMap.set(target, playerIndex)
  });
